...
### 55. Évolution V12 (V12.1) : Enregistrement Multipiste (DAW-Style) & Compensation de Latence Fine
*   **Enregistrement Multipiste DAW-Style (`web/app.js` & `web/style.css`)** :
    - **États Dynamiques** : Conception d'une piste d'enregistrement temporaire intégrée sous la timeline à 3 états interactifs : **🟡 ARMÉ** (bouton orange, VU-mètre actif, réglages d'entrée), **🔴 REC** (rouge clignotant, tracé de la waveform en temps réel), et **🟢 Prise** (vert, lecture et mixage à la volée).
    - **Mixage en Direct** : curseurs de volume et de balance d'entrée (phase d'armement) et de lecture (phase verte temporaire) gérés via l'AudioContext pour un contrôle matériel immédiat.
*   **Résolution des Problèmes de Latence (~1 seconde de décalage) :**
    - **FFmpeg Output Seeking (`src/server.py`)** : Déplacement de l'argument de compensation de latence `-ss` après l'input `-i` lors du transcodage. Élimine les dérives de conteneur sur les WebM non-indexés générés par les navigateurs, assurant une découpe chirurgicale au millième de seconde près sans aucun retard artificiel dans les fichiers WAV/MP3 finaux.
    - **Debounce de Synchronisation HTML5 Audio (`web/app.js`)** : Introduction du verrou temporel `window.guitarLastSeekTime` pour geler les vérifications de dérive temporelle (> 80ms) pendant les 500ms qui suivent un repositionnement ou une commande de lecture. Évite les boucles de seeking infinies asynchrones sur les fichiers WebM bruts, permettant une lecture de piste verte parfaitement fluide.
*   **Persistance et Rechargement Dynamique** :
    - Option de sauvegarde comme Stem dans le répertoire du projet multipiste avec mise à jour automatisée de `library.json` et de `airstep_meta.json`. Rechargement fluide du lecteur sans aucun rechargement complet de la page (F5).
